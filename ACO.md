...
## 멀티에이전트 위임 — 단일 요청, 프롬프트 기반

`aco`로 작업을 위임할 때, 작업이 분해로 이득을 보면 멀티에이전트(서브에이전트 포함) 방식으로 수행하게 한다. 단, 이 멀티에이전트 구성은 **`aco` 요청을 여러 개 보내는 방식이 아니라, provider에 전달하는 단일 프롬프트 안에 직접 명시**하는 방식으로 한다.

- 한 작업에는 `aco ask`(또는 `/aco` · `$aco`)를 **1회만** 호출한다. 역할 분담·병렬 검토·분해 종합은 그 1회 요청의 프롬프트 본문에 지시로 담는다.
- 동일 작업을 쪼개 `aco` 요청을 N개 발사(fan-out)하지 않는다. 여러 요청은 각각 독립 session ID를 가질 뿐, 결과를 합산할 레이어가 없어 중복·불일치·컨텍스트 단절을 만든다.
- 분해가 필요하면 provider 내부에서 일어나도록 프롬프트로 위임한다. 예: "다음 항목을 각각 독립 서브에이전트 관점에서 검토한 뒤 하나로 종합해줘."
- provider 출력은 advisory이며, 최종 종합·판단은 메인 세션이 한다.
