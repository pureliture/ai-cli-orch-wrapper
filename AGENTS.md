...
이 repo의 세부 규칙은 이 파일의 worktree/context-sync 정책과 이 저장소의 공통 작업 원칙을 함께 따른다.
...
- `$aco`는 `$aco status` 같은 task-specific subcommand를 만들지 않는다. 사용자는 자연어로 task를 전달하고(필요 시 provider를 자연어로 명시), 흐름은 내부적으로 `aco ask`로 위임한다. 세션 운영(`aco status`/`result`/`cancel`)은 하부 CLI plumbing이다.
