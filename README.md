...
현재 구현된 공개 npm CLI는 Node wrapper가 담당하며, Go runtime은 `aco delegate`와 blocking
provider 실행 실험을 담당합니다. 주요 provider는 **Gemini**와 **Codex**입니다. 개선 방향과
PR 실행 기준은 [docs/roadmap.md](docs/roadmap.md)와
[docs/pr-implementation-plan.md](docs/pr-implementation-plan.md)를 참고합니다.

## 현재 구현 범위

현재 공개 npm CLI에서 확인할 수 있는 범위:

- `aco pack` / `aco provider setup`으로 command pack과 provider 준비
- `aco sync`로 Claude 기준 자산을 Codex/Gemini 대상 context로 동기화
- `aco run <provider> <command>`로 Gemini/Codex provider 실행
- `aco status`, `aco result`, `aco cancel`로 session lifecycle 조회와 조작
...
`provider setup`은 외부 CLI가 설치되어 있는지 확인한 뒤, 다음 순서로 local credential
readiness를 휴리스틱으로 확인합니다. fallback의 `--version` 실행은 provider binary
availability 확인이며 remote 인증 검증이 아닙니다.
...
- **Node wrapper CLI**: 공개 npm package의 `aco` 명령. command pack 설치, provider setup,
  `aco sync`, `aco run`, 세션 로그와 운영 명령 담당
...
- [PR Implementation Plan](docs/pr-implementation-plan.md)
