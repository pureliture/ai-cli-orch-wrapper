...
| 우선순위 | 작업                                             | 이유                                                |
| -------- | ------------------------------------------------ | --------------------------------------------------- |
| P0       | README 첫 화면을 workflow harness 중심으로 개편  | 첫 30초 안에 가치가 보여야 한다.                    |
| P0       | 루트 `package.json` description 정리             | Gemini-only 인상을 줄여야 한다.                     |
| P0       | `docs/case-study.md` 추가                        | 포트폴리오/평가자용 단일 링크가 필요하다.           |
| P1       | mock provider 또는 no-auth demo path 추가        | 인증 없이 핵심 workflow를 보여줘야 한다.            |
| P1       | `aco doctor` 추가                                | provider/auth/sync 환경 문제를 진단 가능하게 한다.  |
| P1       | `aco sync --diff`, `aco sync --explain` 추가     | context sync의 차별화가 CLI에서 보이게 한다.        |
| P1       | 사용자용 security model과 `.acoignore` 문서 추가 | 외부 provider 실행에 대한 신뢰를 만든다.            |
| P1       | review artifact schema 도입                      | AI 결과를 검토 가능한 산출물로 만든다.              |
| P2       | multi-provider review aggregator                 | 여러 AI 결과를 비교해 의사결정을 돕는다.            |
| P2       | `aco followup --from-session`                    | review 결과를 GitHub issue workflow로 연결한다.     |
| P3       | provider plugin contract                         | 확장성 어필은 core workflow 이후에 다룬다.          |
| P3       | metrics/report command                           | 효과 측정은 artifact와 workflow 안정화 이후에 둔다. |

## 1차 PR 권장 범위

첫 PR은 기능 추가보다 문서 정리로 제한하는 편이 좋다. PR1-PR8a/8b의 상세 실행 기준은
...
### Issue 3: docs: add portfolio case study
...
### Issue 5: feat(cli): add aco doctor
...
### Issue 6: feat(sync): add sync diff and explain modes
...
### Issue 7: feat(result): standardize review artifact output
...
### Issue 8: docs: add security model and .acoignore guide
