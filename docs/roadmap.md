...
| session lifecycle    | Goal 2에서 run/session brief, raw input, prompt, output, error layout을 문서화했다. | structured findings는 future work다.                        |
| doctor               | Goal 2에서 local-only `aco doctor` v1을 추가했다.                                   | remote auth verification이나 provider invoke는 하지 않는다. |
| mock provider        | Goal 2에서 공개 provider registry에 deterministic `mock` provider를 추가했다.       | AI 품질 데모가 아니라 no-auth runtime demo다.               |
| runtime boundary     | `docs/architecture.md`, `docs/contract/go-node-boundary.md`가 있다.                 | 이미 좋은 내용이 있으나 README에서 더 잘 연결해야 한다.     |
| 보안 문서            | Goal 2에서 `docs/security.md`와 `.acoignore.example`을 추가했다.                    | `.acoignore` enforcement와 secret scanning은 future work다. |
...
README와 case study는 "implemented", "planned", "experimental"을 분리해야 한다. Goal 2 기준
`aco ask`, `mock` provider, local-only `aco doctor`, session/run artifact v1은 implemented로
표기하고, aggregator, `.acoignore` enforcement, secret scanning, structured findings는 planned로
표기한다. 포트폴리오 문서에서 과장보다 정확성이 더 중요하다.
...
Goal 2에서 local-only v1을 구현했다. 운영 강화의 다음 단계는 output severity 분류와 더 구체적인
remediation hint다.
...
Goal 2에서 사용자가 실행할 수 있는 공개 `mock` provider를 registry에 추가했다. 인증 없이 다음
흐름이 가능하다.
...
`demo init`은 필요할 때 다음 단계로 둬도 된다.

### 6. review artifact 표준화

수용하되, mock provider와 session 호환성을 먼저 고려한다.

Goal 2의 artifact v1은 기존 호환성을 유지하면서 run-level `ledger.json`과 `brief.md`, session-level
`input.md`, `prompt.md`, `brief.md`, `error.log`를 추가/문서화했다. `findings.json`,
`validation.json`, `aco result --format json`은 provider output normalization이 필요하므로 v2로
분리한다.
...
| Operational hardening | 사용자용 security model과 `.acoignore` 문서 추가 | Goal 2에서 docs/example 완료. Enforcement는 future.  |
...
- 아직 없는 aggregator, `.acoignore` enforcement, secret scanning을 현재 기능처럼 소개
...
Status: Goal 2 implemented the `mock` provider for `aco ask` no-auth demos. `aco run mock review`
compatibility can remain a follow-up if needed.
...
Status: Goal 2 implemented local-only `aco doctor` v1. Future improvements should keep the same
non-network and no-provider-invocation boundary.
