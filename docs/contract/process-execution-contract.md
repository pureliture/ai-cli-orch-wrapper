...
이 문서는 `aco` CLI의 프로세스 실행 계층이 외부 프로바이더 바이너리를 어떻게 생성하고, 제어하고, 종료하는지를 규정한다. 계약의 목적은 외부 기여자가 `aco`의 런타임 동작을 코드 읽기 없이 이해할 수 있도록 하는 것이다.
...
`aco run`과 `aco delegate`는 동기(synchronous)이고 차단(blocking)된다.
...
`cmd.Env`는 호스트의 전체 환경변수 목록에서 **allowlist**에 포함된 변수만 전달한다:
...
- `aco run` 모드: `cmd.Stdout = os.Stdout`
