...
차단되는 변수 예시: `ACO_API_KEY`, `GOOGLE_API_KEY`, `AZURE_OPENAI_KEY`, `COPILOT_TOKEN`, `GEMINI_API_KEY` 및 그 외 모든 환경 변수.

> **Antigravity headless 인증 주의**: `antigravity` provider(`agy`)는 OS Keyring을 통해 인증하며 headless API key 환경 변수를 사용하지 않는다. CI/headless 환경에서 `antigravity`의 인증 방법은 현재 미정(open risk)이다.
...
각 프로바이더 구현체 (`AntigravityProvider`, `CodexProvider`)는 `IProvider` 인터페이스를 구현한다:

- `key`: 프로바이더 키 ("antigravity", "codex")
...
| ------------ | -------------------------------------------- | ------------------- |
| Antigravity  | (없음 — OS Keyring, env key 없음)            | `agy --version`     |
| Codex        | `OPENAI_API_KEY`, `~/.codex/auth.json`       | `codex --version`   |

`antigravity` provider(`agy`)는 OS Keyring을 통해 인증하며 환경 변수 기반 fast-path credential
source가 없다. CI/headless 환경에서 `agy`의 인증 방법은 현재 미정(open risk)이다. Go delegate
runtime의 환경 변수 allowlist에 antigravity 전용 key는 없다.
...
- `GEMINI_API_KEY`, `COPILOT_TOKEN`
- 기타 모든 환경 변수

> `antigravity` provider(`agy`)는 OS Keyring 기반 인증을 사용하므로 전달할 headless key가 없다.
