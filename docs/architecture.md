...
사용자의 명시적 동의를 받은 뒤 Codex/Antigravity 같은 외부 AI CLI에 작업을 위임하고, 결과는
...
문서 기준 주요 real provider는 **antigravity**와 **codex**다. MVP demo와 테스트에는 인증이 필요 없는
...
Go provider registry에는 현재 `codex`, `antigravity`가 등록되어 있다.
...
`aco sync`는 Claude Code 프로젝트 설정을 Codex의 프로젝트 단위 대상 파일로 변환한다.
저장소의 canonical Claude 파일을 읽고, `.aco/sync-manifest.json`(v5)에 해시를 추적하는 관리 산출물을 쓴다. Gemini provider 제거(Phases 1-3)로 `GEMINI.md`는 더 이상 생성 대상이 아니다.
...
`aco sync`는 생성된 Codex 대상 block을 소유하고 `.aco/sync-manifest.json`(v5)에 해시를 기록한다.
