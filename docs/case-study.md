...
> 2026-05-08 Goal 2 update: PR #92 added the consent-gated `aco ask` MVP, mock provider, generic `/aco` command, and run/session artifacts. Goal 2 hardening adds raw input preservation, bounded brief summaries, local-only `aco doctor`, security documentation, and artifact docs. Earlier sections that describe mock provider or `aco doctor` as planned should be read as historical context unless the current implementation section says otherwise.

`ai-cli-orch-wrapper`는 Claude Code 중심의 repo-local harness를 Codex CLI와 Antigravity CLI 대상
...
- Claude Code 기준 자산을 Codex 대상 파일로 동기화하는 `aco sync`
- `aco ask`를 통해 사용자의 `--yes` 동의 뒤 mock/Codex/Antigravity provider에 advisory 작업을 위임하는 Node wrapper CLI
- 실행 결과를 `aco status`, `aco result`, `aco cancel`과 run/session artifact로 추적하는 lifecycle

Goal 2 기준으로 no-auth `mock` provider, local-only `aco doctor`, raw input preservation,
bounded brief summary, security docs, session/run artifact docs가 구현되어 있다. Structured
findings artifact와 multi-provider aggregation은 roadmap에 남아 있다. 이 프로젝트의 목표는 AI가
review를 대체하게 하는 것이 아니라, AI CLI 사용을 검토 가능하고 반복 가능한 개발 workflow로
만드는 것이다.
...
- Claude Code, Codex, Antigravity의 설정과 agent/skill 표면이 완전히 같지 않다.
...
- Codex/Antigravity target context를 생성 산출물로 관리해야 한다.
...
주요 provider는 Antigravity와 Codex다.

### Context Sync

`aco sync`는 Claude Code 기준 자산을 읽고 Codex target으로 변환한다.
...
핵심은 `.claude/`를 사람이 관리하는 기준 자산으로 두고, Codex 대상 파일은
manifest-tracked generated output으로 관리하는 것이다. `GEMINI.md`와 `.gemini/agents/`는
Phases 1-3 마이그레이션으로 Gemini provider가 제거되면서 생성 대상에서 제외되었다. 현재 `--check`, `--dry-run`, `--force`를
...
`aco status`, `aco result`, `aco cancel`은 이 session lifecycle을 조회하거나 조작한다. Goal 2의
`aco ask` artifact v1은 `~/.aco/runs/<run-id>/ledger.json`, run/session `brief.md`, `input.md`,
`prompt.md`, `output.log`, failure/cancel path의 `error.log`를 문서화한다. `findings.json`과
`validation.json`은 provider output normalization이 필요하므로 future work로 분리한다.
...
두고, Codex 파일을 generated target으로 관리한다.
...
`agy --version`, `codex --version` fallback은 binary availability 확인이지 remote auth 검증이
아니다. `aco doctor`도 이 한계를 유지하며 local readiness heuristic만 보고한다.
...
| Claude source -> Codex generated target        | context drift를 줄이고 ownership을 명확히 한다. | 모든 provider 설정을 무손실 변환할 수는 없다.                          |
...
| Mock provider                                  | 인증 없는 demo와 CI 검증이 가능해진다.          | AI 품질 데모가 아니라 deterministic runtime 검증임을 명확히 해야 한다. |
...
- `aco sync --diff`, `aco sync --explain`은 아직 planned work다.
- Structured `findings.json`/`validation.json`은 아직 구현하지 않았다.
- `.acoignore.example`은 policy/example일 뿐이며 enforcement는 아직 구현하지 않았다.
...
현재 hardening 기준과 검증 증거는 [Goal 2 plan ledger](plans/consent-gated-delegation-hardening/)를
따른다. [PR Implementation Plan](pr-implementation-plan.md)은 이전 PR 분할 기준과 남은
후속 후보를 보존하는 historical plan이다.

1. README 첫 화면과 package metadata 정리
2. `aco sync --diff`와 `aco sync --explain`
3. `.acoignore` enforcement와 secret scanning strategy
4. structured findings artifact
5. multi-provider review aggregation
6. follow-up draft / GitHub workflow integration
