...
첫 화면에서 이 저장소가 Antigravity CLI wrapper가 아니라, Claude Code, Codex CLI, Antigravity CLI를
repo-local 개발 워크플로우로 묶는 AI workflow harness임을 이해하게 만든다.

### 배경

현재 README와 root `package.json`은 Antigravity 중심 command pack처럼 읽힐 수 있다. 실제 구현은
...
- root package description이 Antigravity-only 도구처럼 읽히지 않는다.
...
- Must support Codex/Antigravity target context
...
Antigravity/Codex 인증이 없으면 평가자가 직접 실행하기 어렵다. mock provider는 AI 품질을
...
Harness
  ...
  warn AGENTS.md drift detected; run `aco sync --diff`
```
