...
`templates/commands/`, `templates/prompts/`, `templates/tasks/`를 `.claude/` 아래에 복사한다.
...
│   └── tasks/          # aco ask --preset task preset
...
패키지에 포함된 템플릿은 현재 `templates/commands/` 아래의 `/gh-*` 명령과 provider helper 명령,
`templates/prompts/` 아래의 provider prompt, `templates/tasks/` 아래의
`aco ask --preset review/spec-critique/plan-critique/tdd/code-simplify/default` preset을 포함한다.
