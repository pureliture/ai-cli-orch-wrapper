...
```text
User intent
  -> Claude chooses candidate agent from .claude/agents/*.md
  -> instead of spawning that sub-agent, Claude or wrapper policy calls:
     aco delegate <agent-id> --input "..."
  -> ACO reads the same agent spec
  -> ACO resolves provider/model via formatter rules
  -> ACO invokes external CLI
...
- `reasoningEffort`를 provider-specific 값으로 변환
- 최종 해석 불가 시 fallback 제공

즉, agent 파일은 vendor-neutral을 유지하고, formatter는 agent별 프로필 레지스트리가 아니라 vendor-specific translation rule set이 된다.
...
## Semantic Mapping from Claude Fields

참고용 개념 매핑:

| Claude field | ACO field | 비고 |
|-------------|-----------|------|
| `name` | `id` | 거의 직접 대응 |
...
- canonical shared agent spec in `.claude/agents/*.md`
- formatter rule-based translation
- sub-agent spawn intent -> `aco delegate <agent-id>` 치환 가능
