...
- [ ] 1.6 README·CLI `--help`·기여 가이드 등 문서의 제거 진입점 참조를 `/aco`·`$aco` 기준으로 일괄 정리(문서 reframe 일부는 415ba4d에서 완료)
- [ ] 1.7 `npm run check:skill-templates` 통과 확인
...
- [ ] 2.3 미인증 provider 시 setup 안내(가능하면 동의 기반 interactive provisioning 후 원 작업 복귀) 분기 추가
- [ ] 2.4 `templates/commands/aco.md` 동기화
- [ ] 2.5 `.codex/skills/aco/SKILL.md`(`$aco`) 패리티 반영
- [ ] 2.6 금지 subcommand(`status`·`result`·`cancel`·`delegate`) 가드레일 구현 + syntactic integration test로 자동 검증
- [ ] 2.7 `--permission-profile`을 하위 provider에 명시 전파 + 미지원 provider 실행 차단
- [ ] 2.8 최소 컨텍스트 전달(강제 marshaling 없음) 보장 + 검증
- [ ] 2.9 자연어 의도 해석 실패·사용자 취소(Ctrl+C)·중복 동시 호출 정책 구현
...
- [ ] 5.1 비-TTY=대시보드 비활성/요약 로거, `NO_COLOR`=색 제거로 분리 구현
- [ ] 5.2 stdout `brief`를 대시보드 렌더 완료 후 일괄 출력 + 100~200ms throttle/deadband 적용
