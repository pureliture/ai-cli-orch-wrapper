...
5. **동의 후 실행.** 사용자가 확인하면:
   ...
   ```bash
   aco ask --providers <provider> --task "<태스크>" --input "<입력>" --yes --runtime-banner --host codex
   ```
...
aco ask --providers antigravity --task "<태스크>" --input "<텍스트>" --yes --runtime-banner --host codex
```
...
# 동의 후 실행
aco ask --providers mock --task "auth 모듈 아키텍처 리뷰" --input "$(cat docs/architecture.md)" --yes --runtime-banner --host codex
